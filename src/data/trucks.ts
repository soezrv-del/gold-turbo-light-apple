// Tow-vehicle trim database ported from RvTOW.
...
export const TRUCK_DB: Record<string, Record<string, Record<string, TruckModel>>> = {
  '2025': {
    ford: {
      'F-150': {
        trims: [
...
export const HITCH_CLASSES = [
  ...
  { cls: 'Class III', maxTow: 8000, color: '#0077CC', desc: 'Standard — travel trailers up to 30ft' },
  { cls: 'Class IV', maxTow: 14000, color: '#FFFFFF', desc: 'Heavy-duty — large 5th wheels & TTs' },
  { cls: 'Class V', maxTow: 20000, color: '#AACCFF', desc: 'Gooseneck/commercial — all RV types' },
];

export const TRUCK_MAKE_LABELS: Record<string, string> = {
  ford: "Ford",
  ram: "Ram",
  chevrolet: "Chevrolet",
  gmc: "GMC",
...
export function truckYears(): string[] {
  const years: string[] = [];
  for (let y = 2026; y >= 2010; y--) years.push(String(y));
  return years;
}

export function truckMakes(year: string): string[] {
  const d = TRUCK_DB[year] ?? TRUCK_DB["2024"] ?? TRUCK_DB["2025"];
  return d ? Object.keys(d) : [];
}

export function truckModels(year: string, make: string): string[] {
  const d = TRUCK_DB[year] ?? TRUCK_DB["2024"] ?? TRUCK_DB["2025"];
  if (!d) return [];
...
export function truckTrims(year: string, make: string, model: string): TrimSpec[] {
  const yearKey = TRUCK_DB[year] ? year : TRUCK_DB["2024"] ? "2024" : "2025";
...
export function hitchForGvwr(gvwr: number) {
  for (const h of HITCH_CLASSES) {
    if (gvwr <= h.maxTow) return h;
  }
